...
test('formats amount input using Indonesian separators', () => {
  ...
  assert.equal(formatPriceAmountInput('1,999,000', 'id'), '1,999,000')
...
test('formats amount input using English separators', () => {
  ...
  assert.equal(formatPriceAmountInput('1.999.000', 'en'), '1.999.000')
...
test('normalizes localized amount input without treating group separators as decimals', () => {
  ...
  assert.equal(normalizeLocalizedPriceAmountInput('3,1231', 'id'), '31231')
  assert.equal(normalizeLocalizedPriceAmountInput('3,123123', 'id'), '3123123')
...
test('accepts comma as a decimal separator for English decimal drafts', () => {
  assert.equal(normalizeLocalizedPriceAmountInput('14,', 'en'), '14.')
  assert.equal(normalizeLocalizedPriceAmountInput('14,5', 'en'), '14.5')
  assert.equal(normalizeLocalizedPriceAmountInput('14,25', 'en'), '14.25')
  assert.equal(isPriceAmountDraftValid('14,5', 'en'), true)
})
...
test('validates draft amount input before formatting', () => {
  ...
  assert.equal(isPriceAmountDraftValid('3,1231', 'id'), true)
